...
// Set的值 和 Map 的键 如果在其他引用的地方发生改变，并不会影响他们；
 ...
 */
  ...
  * Map 的键可以是任意类型，WeakMap 的键只能是对象类型
    WeakMap 键名所指向的对象，不计入垃圾回收机制；就是说如果这个对象在别的地方没有引用，垃圾回收的时候就会把它回收掉；WeakMap 中就不存在这个键名 的 键值 对了；
    get()、set()、has()、delete()只有这四个方法
    用处：
        在 DOM 对象上保存相关数据；键名是dom对象，键值是相应的数据；用来存储；
        数据缓存； 用来管理数据；因为他的回收机制，所以我们不用对缓存的数据进行关系；
        类的私有属性，其中的一种方法：就是类本身this 作为键名；私有值作为 键值；
